...
const Login = () => {
  ...
  const handleLogin = async ()=>{
    ...
    //->await inorder to fetch the data, URL+(route path) , than data as an object
      
    try {
        const res = await axios.post(URL+"/api/auth/login", {email, password}, {withCredentials:true})
        // console.log(res.data)
        // once we set setUser res.data, after that we can use this data by user in our application
        setUser(res.data)
        navigate("/")
  ...
  return (
    ...
    <div className="flex items-center justify-between px-6 md:px-[200px] py-4">
        <h1 className="text-lg md:text-xl font-extrabold"><Link to="/">TravelExpo</Link></h1>
